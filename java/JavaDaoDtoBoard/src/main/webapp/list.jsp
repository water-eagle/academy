...
<body>
	...
	// 만약 블럭 마지막 페이지 번호가 전체 페이지 마지막 번호보다 크면 마지막 페이지번호를 페이지 마지막 번호로 저장하는 예외 처리 하기
	if (blockEndNo > totalPage) {
		blockEndNo = totalPage;
	}
	
	// {블럭 처리 - 5/9} (이전/다음) 관련 초기화 처리
	// 현재 블럭에서 이전 다음을 눌렀을 때 넣을 값을 일단 초기화하기
	// 예: <이전 [1][2][3][4][5] 다음>
	int prevPage = 0;
	int nextPage = 0;
	
	// {블럭 처리 - 6/9} (이전/다음) 관련 계산 처리
	// 현재 블럭에서 이전/다음이 가능한지 계산하고, 가능 여부를 저장하기
	/* 이전 */
	...
	if (currentBlockNo < totalBlock) { // 현재 블럭이 마지막 블럭보다
		hasNext = true; // 다음 블럭 가기 기능
	...
	// {블럭 처리 - 7/9} (이전/다음) (이전) 처리
	// 이전 블럭 이동이 가능하면 미리 계산한 이전 블럭 이동 시 이동할 페이지 번호를 링크에 전달하기
	if (hasPrev) {
	...
	// {블럭 처리 - 8/9} 기존의 제한 없던 페이지리스트 나열을 ex. [1][2][3].....[n] 블럭 적용하기
	// 현재 블럭의 페이지 시작번호와 끝번호를 이용하여 반복문의 시작값 끝값으로 하고 이 값을 페이지 번호로 출력하기
	// for (int i = 1; i < pageMaxNumber; i++) { // << 이전 반복문을 
	for (int i = blockStartNo; i <= blockEndNo; i++){ // << 이렇게 바꿈
		if (nPageNum == i) { // 현재 페이지랑 번호가 같으면 링크 안 걸어줌
			%>[<%=i %>]<%
		} else {
			//<검색처리 5/5> 하단 페이지 링크 테스트 출력
			if (searchWord == null) { // <1>일반 리스트
				%><a href="list.jsp?page=<%=i %>">[<%=i %>]</a><%
			} else { // <2> 검색어로 검색한 리스트
				// *중요* 한글 검색어를 전달할 때는 url인코딩을 해줘야함. 영어는 괜찮음. 그래도 다 해주고
				String urlEncodedSearchWord = java.net.URLEncoder.encode(searchWord, java.nio.charset.StandardCharsets.UTF_8.toString());
			%><a href="list.jsp?page=<%=i %>&word=<%=urlEncodedSearchWord%>">[<%=i %>]</a><%
			}
		}
	}
	
	// {블럭 처리 - 9/9} (이전/다음)의 (다음) 처리
